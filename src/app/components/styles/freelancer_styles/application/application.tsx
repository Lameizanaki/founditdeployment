...
const DefaultAvatar: React.FC = () => (
  <svg className="w-full h-full" viewBox="0 0 40 40" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    <path d="M8 32C8 26.4772 12.4772 22 18 22H22C27.5228 22 32 26.4772 32 32V32H8V32Z" fill="#9CA3AF"/>
  </svg>
);
...
const StatusBadge: React.FC<{ status: string }> = ({ status }) => {
  const statusConfig: Record<string, { bg: string; text: string; label: string }> = {
    shortlisted: { bg: 'bg-purple-600', text: 'text-white', label: 'Shortlisted' },
    'pending': { bg: 'bg-gray-600', text: 'text-white', label: 'Pending Review' },
    'under-review': { bg: 'bg-blue-600', text: 'text-white', label: 'Under Review' },
  ...
  return (
    <div className={`${config.bg} ${config.text} px-3 py-1.5 rounded-full text-xs font-medium inline-flex items-center gap-1.5`}>
...
const StatsCard: React.FC<{ title: string; count: number; icon: React.ReactNode; bgColor: string; iconColor: string }> = ({ 
  title, 
  count, 
  icon, 
  bgColor,
  ...
  <div className="bg-white rounded-xl p-6 border border-gray-100">
    <div className="flex items-start justify-between">
      ...
      <div className={`${bgColor} w-12 h-12 rounded-xl flex items-center justify-center ${iconColor}`}>
        {icon}
      </div>
    </div>
  </div>
  ...
  <div className="bg-white rounded-xl p-6 border border-gray-100">
    <div className="flex items-start justify-between mb-4 gap-4">
      <div className="flex-1 min-w-0">
        <h3 className="text-base font-semibold text-gray-900 mb-2">{application.title}</h3>
        <div className="flex items-center gap-2 text-sm text-gray-500">
          <span>Posted by</span>
          <div className="w-5 h-5 rounded-full overflow-hidden flex-shrink-0">
            <DefaultAvatar />
          </div>
          <span className="text-gray-700 font-medium">{application.postedBy.name}</span>
        </div>
      </div>
    ...
    <div className="grid grid-cols-3 gap-6 mb-5">
      <div className="flex items-start gap-2">
        ...
        <div>
          ...
          <div className="text-sm font-semibold text-gray-900">{application.rate}</div>
        </div>
      </div>
      ...
      <div className="flex items-start gap-2">
        ...
        <div>
          ...
          <div className="text-sm font-semibold text-gray-900">{application.delivery}</div>
        </div>
      </div>
      ...
      <div className="flex items-start gap-2">
        ...
        <div>
          ...
          <div className="text-sm font-semibold text-gray-900">{application.applied}</div>
        </div>
      </div>
    </div>

    <div className="mb-4">
      <div className="text-xs font-semibold text-gray-900 mb-1.5">Cover Letter</div>
      <div className="text-sm text-gray-600 leading-relaxed">{application.coverLetter}</div>
    </div>

    <div className="flex flex-wrap gap-2 mb-5">
      {application.skills.map((skill, index) => (
        <span key={index} className="bg-gray-50 border border-gray-200 px-3 py-1.5 rounded-lg text-xs text-gray-700 font-medium">
          {skill}
        </span>
      ))}
    </div>
    ...
    <div className="flex items-center justify-between pt-4 border-t border-gray-100">
      <div className="text-xs text-gray-500">
        Job Budget: <span className="text-gray-900 font-semibold">{application.budget}</span> • {application.proposals} proposals
      </div>
      <p 
...
export default function ApplicationPage() {
  ...
  const sortOptions = ['Most Recent', 'Oldest First', 'Highest Rate', 'Lowest Rate'];
  ...
  const [applications] = useState<Application[]>([
    {
      ...
      delivery: '14 days',
      applied: 'Mar 15, 2024',
      coverLetter: 'I have over 5 years of experience in e-commerce design and have worked with brands similar to yours. I can deliver a modern, conversion-optimized design that aligns with your brand.',
    ...
    },
    {
      ...
      delivery: '21 days',
      applied: 'Mar 14, 2024',
      coverLetter: 'I specialize in mobile app design with a focus on user experience. I\'d love to help bring your app vision to life.',
    ...
    },
    {
      ...
      delivery: '10 days',
      applied: 'Mar 12, 2024',
      coverLetter: 'I have a strong portfolio in brand identity work and would love to create a unique identity for your brand.',
      skills: ['Branding', 'Logo Design', 'Visual Identity', 'Adobe Illustrator'],
      budget: '$3,000 - $5,000',
      proposals: 25,
      status: 'rejected'
    },
    {
      ...
      delivery: '18 days',
      applied: 'Mar 10, 2024',
      coverLetter: 'I have extensive experience designing complex dashboards for SaaS products. I understand the importance of data visualization and user-friendly interfaces.',
    ...
    },
    {
      ...
      delivery: '7 days',
      applied: 'Mar 8, 2024',
      coverLetter: 'I can design and develop a high-converting landing page for your campaign. My approach focuses on clear CTAs and modern design.',
