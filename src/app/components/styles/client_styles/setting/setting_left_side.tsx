...
const sidebarItems1: SidebarItem[] = [
  ...
  { name: 'Invoices & Receipts', id:'#',
    icon: (
        <svg className="w-6 h-6 ml-1" viewBox="0 0 18 20" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M4.6 5.8H13M4.6 10.6H13M2.56 1H15.04C15.9016 1 16.6 1.80589 16.6 2.8V19L14 17.2L11.4 19L8.8 17.2L6.2 19L3.6 17.2L1 19V2.8C1 1.80589 1.69844 1 2.56 1Z" stroke="black" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
        </svg>
    )  
  },
...
const sidebarItems2 : SidebarItem[]= [
    ...
    icon: (
        <svg className="w-7 h-7" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M20 21C20 19.6044 20 18.9067 19.8278 18.3389C19.44 17.0605 18.4395 16.06 17.1611 15.6722C16.5933 15.5 15.8956 15.5 14.5 15.5H9.5C8.10444 15.5 7.40665 15.5 6.83886 15.6722C5.56045 16.06 4.56004 17.0605 4.17224 18.3389C4 18.9067 4 19.6044 4 21M16.5 7.5C16.5 9.98528 14.4853 12 12 12C9.51472 12 7.5 9.98528 7.5 7.5C7.5 5.01472 9.51472 3 12 3C14.4853 3 16.5 5.01472 16.5 7.5Z" stroke="black" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
        </svg>
        ) 
    },
    { name: 'Security', id:'#',
        ...
        <div>
            <div className="text-gray-600 flex flex-col gap-y-6">
                {sidebarItems1.map((item, index) => (
                    <p key={index} className="flex items-center gap-x-3 px-3 cursor-pointer hover:text-blue-500">
                        {item.icon}
                        {item.name}
                    </p>
                ))}
        ...
        <div>
            <div className="text-gray-600 mt-8 flex flex-col gap-y-6">
                {sidebarItems2.map((item, index) => (
                    <p
                        key={index}
                        className="flex items-center gap-x-3 px-3 cursor-pointer hover:text-blue-500"
                    >
                        {item.icon}
                        {item.name}
                    </p>
